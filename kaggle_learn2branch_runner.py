#!/usr/bin/env python3
"""Run the Learn2Branch/Ecole setcover pipeline in a Kaggle notebook.

This script is meant to be invoked with the Kaggle l2b conda/micromamba
Python. Outputs and checkpoints are written under /kaggle/working so they
can be saved as Kaggle notebook output.
"""
...
ROOT = Path(os.environ.get("L2B_ROOT", "/kaggle/working"))
ECOLE_REPO = ROOT / "learn2branch-ecole"
ORIGINAL_REPO = ROOT / "learn2branch"
ARTIFACT_DIR = Path(os.environ.get("L2B_ARTIFACT_DIR", str(ROOT / "learn2branch_setcover_100k")))
...
INSTANCE_ARCHIVE = ARTIFACT_DIR / "instances_setcover_500r_1000c_0.05d.tar.gz"
SAMPLE_ARCHIVE = ARTIFACT_DIR / "samples_500r_1000c_0.05d.tar.gz"
LATEST_SAMPLE_CHECKPOINT = ARTIFACT_DIR / "partial_samples_latest_500r_1000c_0.05d.tar.gz"
LATEST_SAMPLE_COUNT = ARTIFACT_DIR / "partial_samples_latest_count.txt"
...
def latest_partial_sample_archive() -> Path | None:
    archives = sorted(
        list(ARTIFACT_DIR.glob("partial_samples_500r_1000c_0.05d_*.tar.gz"))
        + list(ARTIFACT_DIR.glob("partial_samples_latest_500r_1000c_0.05d.tar.gz"))
        + list(Path("/kaggle/input").glob("**/partial_samples_500r_1000c_0.05d_*.tar.gz"))
        + list(Path("/kaggle/input").glob("**/partial_samples_latest_500r_1000c_0.05d.tar.gz"))
    )
    return archives[-1] if archives else None


def find_existing_archive(name: str) -> Path | None:
    candidates = [ARTIFACT_DIR / name]
    candidates.extend(Path("/kaggle/input").glob(f"**/{name}"))
    for candidate in candidates:
        if candidate.exists() and candidate.stat().st_size >= MIN_INSTANCE_ARCHIVE_BYTES:
            return candidate
    return None
...
def checkpoint_samples(label: str) -> None:
    ...
    print(f"[checkpoint] {label}: saving {count} samples to artifact dir", flush=True)
...
def restore_artifact_checkpoints() -> None:
    instances_dir = ECOLE_REPO / "data" / "instances" / "setcover"
    if not instances_dir.exists():
        archive = find_existing_archive("instances_setcover_500r_1000c_0.05d.tar.gz")
        if archive is not None:
            extract_archive(archive, instances_dir.parent)
...
def save_artifacts() -> None:
    ...
    if (model_dir / "train_params.pkl").exists():
        shutil.copy2(model_dir / "train_params.pkl", ARTIFACT_DIR / "train_params_seed0.pkl")
    if (model_dir / "train_log.txt").exists():
        shutil.copy2(model_dir / "train_log.txt", ARTIFACT_DIR / "train_log_seed0.txt")
...
def checkpoint_training(label: str) -> None:
    ...
    copied = []
    if (model_dir / "train_params.pkl").exists():
        shutil.copy2(model_dir / "train_params.pkl", ARTIFACT_DIR / "train_params_latest.pkl")
        copied.append("train_params_latest.pkl")
    if (model_dir / "train_log.txt").exists():
        shutil.copy2(model_dir / "train_log.txt", ARTIFACT_DIR / "train_log_latest.txt")
...
def main() -> None:
    ...
    if ARCHIVE_FINAL_SAMPLES:
        ...
        print("skipping full pre-training sample archive on Kaggle to save disk", flush=True)
    ...
    run_training_with_monitor(
        [
            ...
            str(SEED),
            "-g",
            str(0 if torch.cuda.is_available() else -1),
            "--max-epochs",
            str(MAX_EPOCHS),
            "--epoch-samples",
            str(EPOCH_SAMPLES),
    ...
    save_artifacts()
    print(f"artifacts saved in {ARTIFACT_DIR}", flush=True)
